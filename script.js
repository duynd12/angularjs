...
myApp.controller('GreetingController', ['$scope', function($scope) {
  ...
  $scope.restaurents.forEach(item => {
        item.numPlace = [];

      $scope.numBac = item.place.mien.bac?.address.length != undefined ? item.place.mien.bac?.address.length: 0;
      $scope.numTrung = item.place.mien?.trung?.address.length != undefined ? item.place.mien.trung?.address.length: 0;
      $scope.numNam = item.place.mien.nam?.address.length != undefined ? item.place.mien.nam?.address.length:0;

      item.numPlace.push($scope.numBac+ $scope.numNam + $scope.numTrung);
  });
  console.log('123');
}]);